...
*/
select h.Point, h.Date, sum(h.Out), sum(h.Inc) from
(
	select o.Point Point, o.Date Date, sum(o.Out) Out, NULL Inc
	from outcome o
	group by o.Point, o.Date
	union all
	select i.Point, i.Date, NULL Out, sum(i.inc) Inc
	from income i
	group by i.Point, i.Date
) h
group by h.Point, h.Date
